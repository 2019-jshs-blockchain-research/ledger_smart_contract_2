  ...
  lendSchoolLedger: function() {
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.SchoolLedger.deployed().then(function(instance) {
        ...
        return instance.lendSchoolLedger(id, grade, kclass, date, kcount, web3.toHex(nameUtf8Encoded), { from: account, value: price*date });
      }).then(function() {
        $('#grade').val('');
        $('#class').val('');
        $('#date').val('');
...
$(function() {
  ...
  $('#lenderInfoModal').on('show.bs.modal', function(e) {
    ...
    App.contracts.SchoolLedger.deployed().then(function(instance) {
      ...
      $(e.currentTarget).find('#lenderDate').text(lenderInfo[4]);
      $(e.currentTarget).find('#lenderCount').text(lenderInfo[5]);
      $(e.currentTarget).find('#lenderName').text(utf.decode(web3.toUtf8(lenderInfo[6])));
    }).catch(function(err) {
      console.log(err.message);
    })
  });
